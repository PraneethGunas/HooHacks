...
export default function QueryInput({ onSubmit, onMockSubmit }: QueryInputProps) {
  ...
  return (
    <section className="relative min-h-screen w-full grid place-items-center px-4 py-10 sm:px-8 sm:py-12">
      ...
      />
      <div className="pointer-events-none absolute right-6 top-8 hidden rounded-full border border-(--fun-pink)/35 bg-(--fun-pink)/10 px-3 py-1 text-xs text-(--fun-pink) md:block">
        7 Agents • &gt;$50B Focus
      </div>
      <div className="mx-auto w-full max-w-3xl enter-card text-center">
        <div className="mb-8 flex items-center justify-center gap-3 text-base font-medium text-white/80">
          ...
          <span className="tracking-tight text-[40px] leading-none font-semibold text-foreground" style={{ fontFamily: "var(--font-wordmark), serif" }}>
        ...
        <div className="rounded-2xl border border-white/12 glass-card p-4 shadow-[0_8px_30px_rgba(0,0,0,0.28)]">
          <textarea
            ...
            rows={3}
            className="w-full resize-none bg-transparent text-[17px] leading-relaxed font-medium text-foreground outline-none placeholder:text-white/25"
          />
            ...
            <div className="flex items-center gap-2">
              {onMockSubmit && (
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/4 px-4 py-2.5 text-[14px] font-medium text-white/50 transition hover:border-white/20 hover:text-white/70"
                >
                  Demo
                </button>
              )}
              <button
                ...
                className={cn(
                  "flex items-center gap-2 rounded-xl bg-linear-to-r from-[#ffe4a6] via-[#ffd06e] to-[#ffb870] px-5 py-2.5 text-[15px] font-semibold text-[#101319] transition",
                  canRun
          ...
          <div className="flex flex-col gap-2">
            {EXAMPLE_QUERIES.map((eq) => (
              <button
                key={eq}
                onClick={() => { setQuery(eq); }}
                className="group text-left rounded-xl border border-white/6 bg-white/2 px-4 py-3 text-[14px] text-white/40 transition hover:border-white/15 hover:bg-white/4 hover:text-white/70"
              >
                <span className="text-white/15 group-hover:text-(--accent-gold) transition mr-2">&rarr;</span>
                {eq}
              </button>
            ))}
          </div>
